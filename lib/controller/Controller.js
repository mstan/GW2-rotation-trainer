const notifier = require('node-notifier'); // required for toast notification
const path = require('path'); // required for toast notification
...
function keypress(command, value) {
	...
	return new Promise( (resolve) => {
		...
		notifier.notify({message:command.name, sound:false, appID:'rotation-trainer', id:1}); // toast notification
